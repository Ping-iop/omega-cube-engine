"""
Omega-Cube MCP Server — FastMCP implementation (motor v2).

Exposes Omega-Cube Engine V2 tools via MCP for plug-and-play
integration with Hermes Agent, Claude, GPT, and any MCP-compatible agent.

MIGRACIÓN 2026-08-25 (MEJORAS.md P0.1): servidor migrado de engine v1 a
OmegaCubeEngineV2 (mejoras arXiv 2026: TypedSchema/VirtualSet, resumen
jerárquico H²MT, BoundaryController PAGE-RAG, detector de alucinación,
índice HNSW O(log n)). Memoria: omega_cube_memory_v2.json (auto-migra
desde v1 si no existe). Guardado atómico + anti-duplicación de axiomas.

Tools:
- omega_cube_query          — Multi-mode graph retrieval (v2: hierarchical/holographic/diffusion/combined)
...
# Singleton engine v2 — auto-carga memory/omega_cube_memory_v2.json (o migra desde v1).
# FIX 2026-08-09 (22:05): el load() extra duplicaba axiomas (→ corrupción 926MB).
# FIX 2026-08-25 (P0.3): guard anti-duplicación también en engine_v2.load().
...
def omega_cube_query(
    ...
    mode: retrieval mode - 'hierarchical' (HNSW + depth-aware rerank, default),
          'holographic' (multisignal), 'diffusion' (scoped diffusion),
          'combined' (multisignal + domain boost)
    ...
    try:
        ...
        return f"❌ Invalid mode: {e}. Valid modes: hierarchical, holographic, diffusion, combined"

    if not results:
        return f"🔍 No results for '{query}' (mode: {mode})"

    lines = [f"🔍 Omega-Cube v2: '{query}' ({mode} mode, {len(results)} results)\n"]
    for r in results:
...
def omega_cube_learn(
    ...
    node_type: AXIOM, CONCEPT, INSTANCE, SESSION or SUMMARY (default: CONCEPT)
    ...
    try:
        ...
        # VirtualSet pre-execution check del schema tipado de v2
        return f"❌ Tipo inválido rechazado por el schema: {e}"
...
def omega_cube_stats() -> str:
    ...
    hnsw = s.get("hnsw", {})
    lines = [
        "📊 Omega-Cube Engine v2 Statistics",
        "=" * 45,
        ...
        f"📁 Memory: {engine.memory_dir}",
        f"🧬 HNSW index: {hnsw.get('size', '?')} vectors | "
        f"ops bloqueadas por schema: {s.get('invalid_ops_blocked', 0)} | "
        f"abstenciones: {s.get('abstained_results', 0)} | "
        f"sesgos detectados: {s.get('bias_detections', 0)}",
    ]
    return "\n".join(lines)
...
if __name__ == "__main__":
    # FIX 2026-08-25: el banner va a STDERR — stdout es el canal JSON-RPC del
    # server stdio; cualquier print() a stdout corrompe el protocolo MCP.
    s = engine.stats()
    print(
        f"🚀 Omega-Cube MCP Server v2 | nodos: {s['total_nodes']} | "
        f"axiomas: {s['axioms']} | memoria: {engine.memory_dir}",
        file=sys.stderr,
    )
    mcp.run()
