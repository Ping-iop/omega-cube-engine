...
class OmegaCubeEngine:
    ...
    def load(self, path: str = None) -> bool:
        ...
        # FIX P0.4 (2026-08-25): aplicar la dimensión guardada ANTES de
        # recodificar firmas. Antes la dim se aplicaba al final y solo si las
        # firmas estaban vacías → al migrar holographic_dim (256→2048) el
        # encoder quedaba a 2048 pero las firmas se recalculaban a 256
        # (desalineación encoder/firmas).
        config = data.get("config", {})
        if "holographic_dim" in config:
            self.holographic.dim = int(config["holographic_dim"])
        ...
        # FIX P0.4 (2026-08-25): se recalculan SIEMPRE con la dim vigente — el
        # condicional antiguo ("if not signature") dejaba firmas huérfanas de
        # otra dimensión tras una migración de holographic_dim.
        for node in self.nodes.values():
