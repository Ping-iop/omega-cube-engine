#!/usr/bin/env python3
"""
MARP + Omega-Cube + Gemma 4 31B INTEGRATION TEST
Prueba el pipeline completo con Gemma 4 31B como worker.
"""
...
WURL = "http://127.0.0.1:8082/completion"
...
# ─── Omega-Cube Init ──────────────────────────────────────────────
print("=" * 72)
print("  MARP + OMEGA-CUBE + GEMMA 4 31B")
print("  Router: Qwen0.8B (:8084)  |  Worker: Gemma 4 31B (:8082)")
print("=" * 72)
...
for i, (query, expected_domain, followup) in enumerate(tests):
    ...
    try:
        ...
        print(f"  📡 Router: ❌ {ex}")
        doms = ["general"]; rt = 0; rtimes.append(0); hit = False
    ...
    # 3. WORKER (Gemma 4 31B via /completion)
    try:
        gemma_prompt = f"<bos><start_of_turn>user\nAs a {doms[0]} expert: {followup}\nContext: {ctx_str[:150]}<end_of_turn>\n<start_of_turn>model\n"
        r2 = requests.post(WURL, json={"prompt": gemma_prompt, "max_tokens": 80, "temperature": 0.3}, timeout=30)
        wt = (time.perf_counter() - t0) * 1000 - rt - ct
        ans = r2.json().get("content", "")
        ans = re.sub(r'<\|channel>\w+\n?\|?\n?', '', ans).replace('<channel|>', '').strip()
        wtimes.append(wt)
        print(f"  ⚡ Gemma: {wt:.0f}ms → \"{ans[:60].strip()}{'...' if len(ans)>60 else ''}\"")
    except Exception as ex:
        print(f"  ⚡ Gemma: ❌ {ex}")
        wt = 0; wtimes.append(0); ans = ""
...
# ─── Summary ──────────────────────────────────────────────────────
print("\n" + "=" * 72)
print("  📊 RESULTADOS: MARP + OMEGA-CUBE + GEMMA 4 31B")
print("=" * 72)
print(f"\n  🎯 Router Accuracy:  {corr}/{len(tests)} ({corr*100//len(tests)}%)")
rt_avg = sum(rtimes)/len(rtimes) if rtimes else 0
print(f"  📡 Router avg:       {rt_avg:.0f}ms")
wt_avg = sum(wtimes)/len(wtimes) if wtimes else 0
print(f"  ⚡ Gemma avg:        {wt_avg:.0f}ms (35.5 t/s)")
pt_avg = sum(ptimes)/len(ptimes) if ptimes else 0
print(f"  ⏱  Pipeline avg:    {pt_avg:.0f}ms")
print(f"\n  📦 Omega-Cube:       {e.stats()['total_nodes']} nodos, {e.stats()['avg_retrieval_time_ms']:.1f}ms retrieval")
print(f"\n  🔧 Router:           Qwen0.8B (:8084)  ~700MB VRAM")
print(f"  🔧 Worker:           Gemma 4 31B (:8082)  ~20GB VRAM")
print(f"  💾 VRAM total:       ~22.8/24.6 GB (93%)")
print("=" * 72)
